...
export default function Controls() {
    ...
    const handlePlaylistChange = async (event: ChangeEvent<HTMLSelectElement>) => {
        ...
        if (!selected) return;
        try {
            let url = `/api/playlist?playlist=${encodeURIComponent(selected)}`;
            
            if (selected === 'like' && connected && wallet) {
                const address = await wallet.getChangeAddress();
                const stake = resolveRewardAddress(address);
                url += `&stakeAddress=${encodeURIComponent(stake)}`;
            }
    ...
    const playNext = useCallback(() => {
        if (songs.length > 0) {
         ...
         if (!audio){ alert("Select a Playlist.")}
    ...
    const playPrevious = useCallback(() => {
        ...
        if (audio && index === 0){alert("First Song in Playlist.")}
        if (!audio){ alert("Select a Playlist.")}
    }, [songs.length, index, shuffled, changeSong]);
        ...
        if (audio) {
            ...
            const handleEnded = async () => {
                ...
                if (playlistCompleted) {
                    console.log('Playlist completed, not recording play');
                    setIsPlaying(false);
                } else {
                    ...
                    if (currentSong && currentSong.song_id) {
                        ...
                        try {
                            const result = await recordPlay(currentSong.song_id.toString());
    ...
    }, [volume, playNext, playlistCompleted, songs, index]);
        ...
        if (wallet) {
            try {
                ...
                if (connected){
                    ...
                    newPlaylists.push( {value:'like', text:"Favorites"})};
    ...
    return (
        <div className="border-[1px] border-zinc-400 z-40 bg-zinc-500/70  w-[250px] rounded-lg my-1 pt-1 mx-auto">
            ...
            <div className="flex mx-auto justify-evenly pb-2 w-[200px]">
                <button className="cursor-pointer" onClick={playPrevious}>
                    <Image className="m-0 w-[40px] h-auto" src="/previous.svg" alt="Previous" width={40} height={40}/>
                </button> 
            ...
            <div className="flex h-12 items-center justify-between px-1">   
                <div onClick={toggleShuffle} title="Shuffle" className="cursor-pointer">
                    <Image className="w-12" src={shuffled ? "/shuffled.svg" : "/shuffle.svg"} width={50} height={50} alt="Shuffle"/>
                </div>
                <select 
                    id="playlistSelect" 
                    className="text-center h-fit text-xs -mt-6 px-1 border-black border-[1px] text-black"
                ...
                <div className="relative">
                    <div id="volume" onMouseEnter={toggleVolumeSlider} className="cursor-pointer ml-1 text-2xl">
                        🔊
                    </div>
                    {showVolumeSlider && (
